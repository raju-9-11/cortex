# cortex-link — TUI Design Specification

> **Author:** UI/UX Design Architect  
> **Date:** 2025-07-15  
> **Status:** DESIGN — ready for implementation review  
> **Scope:** Complete TUI design for the `cortex` interactive terminal client  
...
```
┌─────────────────────────────────────────────────────────────────────┐
│ 🔥 cortex · claude-sonnet-4-20250514 · ses_01J… · 2.1k tokens     │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  You                                                                │
...
│  ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─  │
│                                                                     │
│  cortex [claude-sonnet-4-20250514]> █                                          │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
...
- **Prompt**: `cortex [model]> ` — shows active model at a glance
...
**Enhanced behavior for cortex-link:**
...
```
cortex [claude-sonnet-4-20250514]> Tell me about the
... streaming pipeline
... in this codebase
...
⏎ Sending (3 lines)
...
```
cortex [claude-sonnet-4-20250514]> /mo█
...
Also respect `CORTEX_THEME=light|dark|auto`.
...
via `CORTEX_KEYMAP=vi` (same as `set -o vi` in bash) but are not the default.
...
- History is per-session and persists across REPL restarts (stored in SQLite
  alongside session data, or in `~/.cortex/history`)
...
```
cortex [claude-sonnet-4-20250514]> /mod → /model
```
...
Run `CORTEX_NO_COLOR=1` to test the fully monochrome experience.
...
2. `NO_COLOR=1` or `CORTEX_NO_COLOR=1` → no color, but bold/dim allowed
...
| Auth error | `✗ Authentication failed. Check CORTEX_API_KEY` | Clear instruction |
...
| Connection pooling | Reuse HTTP connections to cortex backend (already done via Go's default transport) |
...
**Compact mode** (`/compact` or `CORTEX_DISPLAY=compact`):
```
...
**Verbose mode** (`/verbose` or `CORTEX_DISPLAY=verbose`):
```
...
format. If verbose debugging is needed, `CORTEX_DEBUG=1` can enable raw output
to stderr.
...
conventions. If a keybinding isn't in bash/zsh, it shouldn't be in cortex.
...
**Goal:** Features that differentiate cortex-link.
...
5. **Pipe integration** — `cat file.go | cortex run "explain this"`
