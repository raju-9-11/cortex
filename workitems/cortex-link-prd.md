# cortex-link — Product Requirements Document
...
7. [Appendix: Cortex Backend Capabilities](#appendix-cortex-backend-capabilities)
...
### 1.2 What cortex-link Is

**cortex-link** is a rich terminal client for the cortex AI backend. It's a single Go binary that provides an interactive, session-aware AI assistant in your terminal — with support for any LLM provider cortex can reach.

**One-liner:** _"Your terminal's AI brain — any model, fully local or cloud, conversations that persist."_

### 1.3 What Makes cortex-link Different
...
| **Persistent, resumable sessions** | Close your terminal, come back tomorrow, type `cortex-link --resume` and pick up exactly where you left off — with full message history. |
| **Privacy spectrum** | Run 100% local with Ollama (zero network calls) or use cloud APIs — your choice, per-session. |
| **Backend-as-a-service architecture** | cortex-link talks to cortex's API — meaning your sessions are also accessible from a future web UI, mobile app, or other clients. Shared state, not siloed. |
| **Built on an OpenAI-compatible API** | cortex's backend speaks the OpenAI protocol. Any tooling that works with OpenAI works with cortex. |

### 1.4 What cortex-link Is NOT
...
**Tertiary:** Teams running a shared cortex backend who want a standardized CLI experience across the org.
...
cortex-link occupies the **provider-agnostic + persistent + local-first** quadrant, which is currently empty.
...
- **Goal:** A single binary he can `scp` to any server and immediately use. Must work with local Ollama or a remote cortex instance.
...
- **Environment:** Shared development infrastructure, internal cortex server
...
- **Goal:** A team-wide CLI that connects to a centralized cortex server. Eventually: shared sessions, auditability, cost tracking.
...
|----|-------|---------------------|
| **US-001** | **As a** developer, **I want to** start a new conversation from my terminal **so that** I can ask an AI questions without leaving my workflow. | **Given** cortex-link is installed and a cortex backend is reachable, **When** I run `cortex-link`, **Then** I see a prompt and can type a message and receive a streamed AI response within 2 seconds of the first token. |
...
| **US-004** | **As a** developer, **I want to** resume a previous conversation **so that** I don't lose context between terminal sessions. | **Given** I had a conversation yesterday, **When** I run `cortex-link --resume` or `/load <session-id>`, **Then** I see the last N messages and can continue the conversation. |
...
| ID | Story | Acceptance Criteria |
|----|-------|---------------------|
| **US-007** | **As a** developer, **I want to** pipe input to cortex-link **so that** I can use it in shell pipelines. | **Given** I run `cat error.log \| cortex-link "explain this error"`, **When** stdin is not a TTY, **Then** cortex-link reads stdin as context, sends the prompt, prints the response to stdout (no TUI chrome), and exits. |
| **US-008** | **As a** developer, **I want to** configure which cortex server to connect to **so that** I can switch between local and remote backends. | **Given** I set `CORTEX_URL=https://cortex.internal.co`, **When** I start cortex-link, **Then** it connects to the remote cortex server using my `CORTEX_API_KEY`. |
...
| **US-010** | **As a** developer, **I want to** set a system prompt **so that** I can customize the AI's behavior per session. | **Given** I start a session, **When** I run `cortex-link --system "You are a Go expert"` or `/system <prompt>`, **Then** that system prompt is stored with the session and used for all subsequent messages. |
| **US-011** | **As an** ops engineer, **I want to** run a one-shot prompt without entering interactive mode **so that** I can script AI calls. | **Given** I run `cortex-link -p "Write a bash script to rotate logs"`, **When** the response completes, **Then** the output is printed to stdout and the process exits with code 0. Session is optionally persisted. |
...
| **Sessions** | Auto-resume last session on launch (configurable) | Reduces friction. `cortex-link` with no args should "just work." |
...
| **Config** | Environment-based config (`CORTEX_URL`, `CORTEX_API_KEY`, `CORTEX_MODEL`) | Must connect to local or remote cortex with zero config files for quick start. |
| **Config** | Config file support (`~/.config/cortex-link/config.yaml` or `cortex-link.yaml`) | Power users need persistent config. |
| **UX** | Status bar showing model, session, connection status | Users must always know what's happening. |
| **UX** | Slash commands (`/help`, `/exit`, `/new`, `/sessions`, `/load`, `/model`, `/system`, `/clear`) | Proven UX pattern from Discord, Slack, existing `cortex chat`. |
...
| **Pipe Mode** | Non-interactive stdin/stdout mode for scripting | Enables `cat file | cortex-link "explain this"` and shell integration. |
| **One-Shot** | `cortex-link -p "prompt"` exits after response | Scriptability. |
...
| **Provider** | Connection health indicator (reconnect on failure) | Resilience for remote cortex servers. |

### 3.3 V3+ — "Agentic"

| Category | Feature | Rationale |
|----------|---------|-----------|
| **Agentic** | File write operations (with diff preview and user approval) | Competitive with Claude Code / Aider. Requires cortex FB-006 (Tool Sandbox). |
| **Agentic** | Shell command execution (sandboxed, approval required) | High risk, high reward. Requires careful security model. |
| **Agentic** | Multi-step tool chains (AI plans → executes → reports) | Full agentic loop. Depends on cortex's tool calling maturity. |
| **Team** | Multi-user sessions (shared cortex server) | Depends on cortex WI-205 (user identity). |
| **Team** | Session sharing and collaboration | Requires auth + RBAC on cortex side. |
...
| **Web UI** | cortex itself will have a web UI (FB-001). cortex-link is terminal-only. |
...
**Decision: cortex-link MUST support both embedded mode (default) AND remote client mode.**

| Mode | How It Works | When to Use |
|------|-------------|-------------|
| **Embedded (default)** | cortex-link imports cortex's Go packages directly. No HTTP server needed. Starts SQLite, registers providers, calls inference — all in-process. | Single-user local usage. Zero config. Just `cortex-link` and go. |
| **Remote client** | cortex-link connects to a running cortex HTTP server via `CORTEX_URL`. Uses REST + SSE. | Team usage, remote servers, when cortex is already running for its web UI. |

**Rationale:**

- **Embedded-first removes the #1 adoption barrier.** If users have to run `cortex` in one terminal and `cortex-link` in another, we've already lost 60% of them. Claude Code doesn't require running a server. Neither should we.
- **Remote mode enables team use cases** and is trivially supported since cortex already has a full HTTP API. It's a `net/http` client, not a new feature.
- **The existing `cortex chat` code already uses embedded mode** — it calls `app.New()`, creates a session manager, and talks to providers directly. cortex-link extends this pattern with a richer TUI.
- **Data integrity:** In embedded mode, cortex-link owns the SQLite file. In remote mode, the server owns it. Never both simultaneously (the SQLite write lock prevents it, but we should warn clearly).
...
// Remote: calls cortex HTTP API via net/http
```

**⚠️ Open question:** Should embedded mode also start the HTTP server so the web UI can connect? **My recommendation: No.** Keep modes separate. If you want both, run `cortex` (server) and `cortex-link --url localhost:8080` (client). Hybrid mode adds complexity for minimal value.
...
**Rationale:**
- cortex's Tool Sandbox (FB-006) doesn't exist yet. Building agentic features in the client before the backend supports tool execution creates a fragile, client-side implementation that will be rewritten.
...
- Sessions are stored in `~/.local/share/cortex-link/cortex-link.db` (embedded mode) or on the cortex server (remote mode)
...
- `/` prefix is universally understood (Discord, Slack, IRC, existing `cortex chat`).
- Double-Enter to send is already established in the existing `cortex chat` REPL — maintain continuity.
...
**Technical note:** The existing `internal/cli/stream.go` handles basic bold/code rendering with a state machine. cortex-link must significantly expand this — adding headers, lists, links, and ideally syntax highlighting (V2). The current approach of raw ANSI codes is correct for streaming; a library like `glamour` (full-document renderer) won't work for progressive rendering.
...
1. CLI flags:        cortex-link --model gpt-4o --url http://localhost:8080
2. Environment vars: CORTEX_URL, CORTEX_API_KEY, CORTEX_MODEL, CORTEX_LINK_THEME
3. Config file:      ~/.config/cortex-link/config.yaml (or $XDG_CONFIG_HOME)
4. Defaults:         embedded mode, auto-detect Ollama, new session
```

**Config file format (YAML):**
```yaml
# ~/.config/cortex-link/config.yaml
url: ""                          # Empty = embedded mode
api_key: ""                      # For remote cortex server
...
data_dir: "~/.local/share/cortex-link"  # SQLite location (embedded mode)
```
...
| **Embedded mode SQLite locking** | Medium | Medium | If a user runs `cortex` (server) and `cortex-link` (embedded) pointing at the same `.db` file, the write lock will cause failures. Detect this (check for lock file) and warn clearly. Use separate default paths. |
...
| **cortex backend API instability** | Low | Medium | cortex has 14 open P0/P1 work items. Some (WI-010: missing SSE fields, WI-004: streaming context lifecycle) directly affect cortex-link. Coordinate: cortex-link MVP should launch *after* cortex sprint-1 is complete. |

### 5.2 Product Risks

| Risk | Severity | Mitigation |
|------|----------|------------|
| **"Why not just use Claude Code?"** | Critical | Claude Code is locked to Anthropic and requires a subscription. cortex-link works with *any* model including free local ones. Lead with the "bring your own model" message. But acknowledge: if the UX isn't at least 70% as polished as Claude Code, users won't switch. **The bar is high.** |
| **"Why not just use the web UI?"** | High | Because the web UI doesn't exist yet (FB-001). And when it does — terminal users want to stay in the terminal. But if the web UI ships first and is great, cortex-link becomes less urgent. **Recommendation:** Ship cortex-link first as the flagship client experience. |
...
4. **Difficult installation.** If it takes more than `brew install cortex-link` or `curl | tar`, adoption drops off a cliff.
5. **Can't connect to their model.** If someone has an Ollama setup and cortex-link can't auto-detect it, they'll uninstall within 60 seconds.
...
3. **Embedded vs. client as default?** Embedded is the right default for adoption, but it means cortex-link ships with a significant chunk of cortex inside it. Binary size will be 15-25MB instead of 5MB. Acceptable for the UX benefit.
...
| Users connecting to remote cortex servers | Team use case is real |
...
## Appendix: Cortex Backend Capabilities (Current State)

### What cortex-link Can Leverage Today

| Capability | cortex Status | cortex-link Impact |
|------------|-------------|-------------------|
...
### What cortex-link Needs from cortex (Dependencies)

| Dependency | cortex Work Item | Blocking? |
|------------|----------------|-----------|
...
| Request body size limits | WI-001 | No — cortex-link controls its own input |
...
### Recommended cortex Sprint-1 Items to Complete Before cortex-link MVP
...
4. **cortex Backend Coordination** — Ensure WI-004, WI-007, WI-010 are completed before cortex-link beta.
